...
def find_latest_checkpoint():
    ...
    candidates.extend(PROJECT_ROOT.glob("checkpoint_*.pt"))
    candidates.extend(PARKOUR_TRAIN_PATH.glob("checkpoint_*.pt"))
    candidates.extend(Path.cwd().glob("checkpoint_*.pt"))
...
def load_model(checkpoint_path, num_actions):
    model = ParkourMLP(input_size=4 * 14, num_actions=num_actions)
...
def main():
    ...
    try:
        ...
        model = load_model(checkpoint_path, len(env.action_table))
