...
EPISODE_LOG_PATH = PROJECT_ROOT / "training_logs" / "episodes.csv"
...
def ppo_update(model, optimizer, buffer, epochs = 4, clip_epsilon= 0.2, batch_size = 64):
    if len(buffer) == 0:
        return
...
def train():
    ...
    try:
        ...
        model = ParkourMLP(input_size, num_actions)
        ...
        while steps_done < total_timesteps:
            ...
            for _ in range(rollout_steps):
                ...
                buffer.add(obs, action, log_prob, reward, value, float(done))
